...
# -------------------------------
st.markdown(
    "<h1 style='text-align: center; color: white;'>🚦 AI Traffic Violation Dashboard (Advanced)</h1>",
    unsafe_allow_html=True
)

# -------------------------------
# JSON File Path (absolute)
# -------------------------------
...
if not df.empty:
    ...
    st.markdown(
        f"<h2 style='color:#00C853;'>📊 Total Violations: {len(df)}</h2>",
        unsafe_allow_html=True
    )
...
st.markdown("---")
st.markdown(
    "<center>Developed with ❤️ for Smart City Traffic Monitoring | © 2025</center>",
    unsafe_allow_html=True
)
